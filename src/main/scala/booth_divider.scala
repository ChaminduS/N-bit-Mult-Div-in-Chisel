...
class booth_divider(N:Int) extends Module{
    val io = IO(new Bundle{
        val signed   = Input(UInt(1.W))
        val dividend = Input(SInt(N.W))
        val divisor  = Input(SInt(N.W))  //M
...
object boothDivider extends App {
  ...
  (new chisel3.stage.ChiselStage).emitVerilog(new booth_divider(64), Array("--target-dir","verilog/"))
